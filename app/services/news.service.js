...
News.get_all = function (result) {
  db.query(
    "SELECT * FROM news inner join topic on news.Topic_Id =topic.Topic_Id ",
    function (err, news) {
      if (err) {
        result("Error");
      } else {
...
News.get_by_newsTitle = function (News_Title, result) {
  db.query(
    'Select * From news where News_Title LIKE "%' + News_Title + '%"',
    function (err, news) {
      total = news.length;
      if (err || news.length == 0) {
        result("nodata");
...
News.getByTopicId = function (id, result) {
  db.query(
    "SELECT * FROM news where Topic_Id = ? Group By News_Id DESC",
    id,
    function (err, news, total) {
      total = news.length;
      if (err || news.length == 0) {
...
News.update = function (p, result) {
  db.query(
    " UPDATE news SET News_Title =? ,News_Content =? ,News_View = ?,News_Cmt =?,News_Url =?,News_Img_Upload =?,Topic_Id =?,News_Index=? ,Created_At =? WHERE News_Id=?",
    [
      p.News_Title,
      p.News_Content,
      p.News_View,
